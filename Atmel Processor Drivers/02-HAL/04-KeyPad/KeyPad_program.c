...
u8 KEYPAD_GetPressedKey(void)
{
	...
	for(RowIdx=0;RowIdx<KEYPAD_ROWS_NUMBER;RowIdx++)
	{
		...
		SetPinValue(Keypad_RowsPort[RowIdx],Keypad_RowsPin[RowIdx],0);
		
		/*Check columns if pressed*/
		for(ColIdx=0;ColIdx<KEYPAD_COLUMNS_NUMBER;ColIdx++)
		{
			PinState=GetPinVal(Keypad_ColumnsPort[ColIdx],Keypad_ColumnsPin[ColIdx]);
		...
		/*Deactivate the current row*/
			SetPinValue(Keypad_RowsPort[RowIdx],Keypad_RowsPin[RowIdx],1);
